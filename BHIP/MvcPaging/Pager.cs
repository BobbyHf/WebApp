...
namespace MvcPaging
{
	public class Pager : IHtmlString
	{
		...
		public virtual PaginationModel BuildPaginationModel(Func<int, string> generateUrl)
		{
			...
			var start = 1;
			var end = pageCount;
            var nrOfPagesToDisplay = this.pageSize;
				...
				foreach (var paginationLink in model.PaginationLinks)
				{
					if (paginationLink.Active)
					{
						...
						else
						{
                            ...
                            linkBuilder.AppendFormat(" pageno=\"" + paginationLink.PageIndex + "\" name=\"Pager\" href=\"{0}\">{1}</a>", paginationLink.Url, paginationLink.DisplayText);
